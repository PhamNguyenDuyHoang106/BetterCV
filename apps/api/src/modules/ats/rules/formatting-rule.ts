...
export class FormattingRule implements AtsRule {
  async evaluate(cvData: any, jobDescription: string): Promise<AtsRuleResult> {
    ...
    if (placeholderFound) {
      score -= 30;
      findings.push("Phát hiện văn bản tạm thời / placeholder chưa hoàn thiện (ví dụ: Lorem Ipsum, [tên công ty]).");
      recommendations.push("Thay thế tất cả các placeholder và văn bản tạm thời bằng thông tin thực tế của bạn trước khi nộp.");
    }
    ...
    if (totalWordCount < 100) {
      ...
      recommendations.push("Bổ sung thêm mô tả dự án và chi tiết kinh nghiệm làm việc để đạt ít nhất 300 từ.");
    } else if (totalWordCount > 1200) {
      score -= 10;
      findings.push(`CV có dung lượng rất lớn (${totalWordCount} từ), có thể gây khó khăn cho ATS hoặc người sàng lọc đọc nhanh.`);
      recommendations.push("Tối ưu hóa từ ngữ, làm nổi bật thành tựu chính và hạn chế mô tả rườm rà để tránh vượt quá 2 trang A4.");
    } else {
      findings.push(`Độ dài CV phù hợp (${totalWordCount} từ), lý tưởng cho 1-2 trang A4.`);
    }
    ...
    if (allMetrics.length > 5 && uniqueMetrics.size === 1) {
      score -= 15;
      findings.push("Phát hiện tỷ lệ phần trăm số liệu giống hệt nhau lặp đi lặp lại. Đây là dấu hiệu của AI tự bịa số liệu (fabrication risk).");
      recommendations.push("Thay thế hoặc đa dạng hóa các số liệu thống kê để phản ánh chính xác kết quả thực tế của bạn.");
    }
