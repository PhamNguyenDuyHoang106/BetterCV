  ...
  async evaluate(cvData: any, jobDescription: string): Promise<AtsRuleResult> {
    ...
    for (const req of requiredSections) {
      if (!sectionTypes.includes(req.type)) {
        ...
        recommendations.push(`Vui lòng thêm phần [${req.name}] vào CV để nhà tuyển dụng có đủ thông tin đánh giá.`);
      } else {
        ...
        if (req.type === "PROFILE") {
          ...
          if (!content.email && !content.phone) {
            score -= 10;
            findings.push("CV thiếu thông tin liên lạc tối thiểu (Email hoặc Số điện thoại).");
            recommendations.push("Bổ sung Số điện thoại và Email liên hệ để nhà tuyển dụng dễ dàng tiếp cận.");
          }
        }
        ...
        if (req.type === "EXPERIENCE") {
          ...
          if (!Array.isArray(items) || items.length === 0) {
            ...
            recommendations.push("Thêm lịch sử kinh nghiệm làm việc chi tiết (tên công ty, vai trò, thời gian).");
          } else {
            const emptyDesc = items.some((item: any) => !item.description || item.description.trim().length < 15);
            if (emptyDesc) {
              score -= 10;
              findings.push("Có vị trí kinh nghiệm thiếu phần mô tả chi tiết công việc hoặc mô tả quá ngắn.");
              recommendations.push("Bổ sung mô tả chi tiết nhiệm vụ và thành tựu (dùng Markdown gạch đầu dòng) cho tất cả vị trí làm việc.");
            }
          }
        }
        ...
        if (req.type === "SKILLS") {
          ...
          if (!Array.isArray(items) || items.length === 0) {
            ...
            recommendations.push("Liệt kê các công nghệ, công cụ, hoặc kỹ năng cốt lõi bạn thành thạo.");
          }
        }
      }
    }
    ...
    if (score === 100) {
      findings.push("Tuyệt vời! Tất cả các phần thiết yếu đều có đầy đủ thông tin chi tiết.");
