...
export class KeywordRule implements AtsRule {
  private readonly COMMON_TECH_KEYWORDS = [
    ...
    "nodejs", "nestjs", "express", "koa", "fastify", "python", "django", "flask", "fastapi",
    "golang", "java", "spring", "springboot", "c#", "dotnet", "asp.net", "php", "laravel",
    "ruby", "rails", "rust", "c\\+\\+", "docker", "kubernetes", "k8s", "aws", "azure", "gcp",
    "devops", "ci/cd", "jenkins", "github actions", "gitlab ci", "terraform", "ansible",
    "postgresql", "mysql", "mongodb", "redis", "elasticsearch", "cassandra", "graphql",
    "rest api", "grpc", "microservices", "agile", "scrum", "kanban", "git", "linux",
    "nginx", "apache", "kafka", "rabbitmq", "security", "oauth", "jwt", "prisma", "sequelize"
  ];
  ...
  async evaluate(cvData: any, jobDescription: string): Promise<AtsRuleResult> {
    ...
    if (!jobDescription) {
      return {
        ...
        recommendations: ["Vui lòng tải lên mô tả công việc (JD) để kiểm tra mức độ từ khóa."]
      };
    }
    ...
    if (jdKeywords.length === 0) {
      return {
        ...
        weight: 0.4,
        findings: ["Không phát hiện từ khóa kỹ thuật phổ biến nào trong mô tả công việc."],
        recommendations: ["Bổ sung thêm các yêu cầu kỹ thuật chi tiết hơn trong mô tả công việc."]
      };
    }
    ...
    findings.push(`Phát hiện ${matchedKeywords.length}/${jdKeywords.length} từ khóa kỹ thuật cốt lõi từ JD xuất hiện trong CV.`);
    ...
    if (missingKeywords.length > 0) {
      findings.push(`Từ khóa còn thiếu: ${missingKeywords.slice(0, 10).join(", ")}${missingKeywords.length > 10 ? "..." : ""}`);
      ...
      findings.push("Tuyệt vời! CV của bạn chứa đầy đủ tất cả từ khóa kỹ thuật yêu cầu trong JD.");
      recommendations.push("Tiếp tục duy trì tính nhất quán của từ khóa xuyên suốt hồ sơ.");
    }
